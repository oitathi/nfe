...
/**
 * Gerar Callback Faltante Scheduled.
 *
 * @author Marcelo Oliveira {marcelo.doliveira@b2wdigital.com}
 * @version 1.0
 */
@Service
public class GerarCallbackFaltanteScheduled extends AbstractScheduled {

    private static final Logger LOGGER = LoggerFactory.getLogger(GerarCallbackFaltanteScheduled.class);
    private static final String TIPO_DOCUMENTO_FISCAL_NFE = FazemuUtils.TIPO_DOCUMENTO_FISCAL_NFE.toUpperCase();

    private static final String KEY = RedisOperationsDao.KEY_SEMAFORO_GERAR_CALLBACK_FALTANTE;
    private static final String LOCK_HOST_EM_PROCESSAMENTO = "LOCK_GERAR_CALLBACK_FALTANTE";
    private static final String LOCK_EM_SELECAO_DE_DOCUMENTOS = "LOCK_GERAR_CALLBACK_FALTANTE_EM_SELECAO_DE_DOCUMENTOS";
    ...
    public void gerarCallbackFaltante() throws UnknownHostException {
        LOGGER.info("gerarCallbackFaltante (scheduled) - INICIO");

        String statusScheduled = parametrosInfraRepository.getAsString(TIPO_DOCUMENTO_FISCAL_NFE, ParametrosInfraRepository.PAIN_SCHEDULED_GERAR_CALLBACK_FALTANTE);
            ...
            listaDocumentosEleitos.stream().forEach(documentoFiscal -> {
                ...
                try {
                    if (documentoFiscal.getChaveAcesso() != null
                            && documentoFiscal.getSituacaoDocumento() != null) {

                        TipoServicoEnum tipoServico = null;
                        if (SituacaoDocumentoEnum.AUTORIZADO.getCodigo().equals(documentoFiscal.getSituacaoDocumento())) {
                            tipoServico = TipoServicoEnum.AUTORIZACAO;
                        } else if (SituacaoDocumentoEnum.CANCELADO.getCodigo().equals(documentoFiscal.getSituacaoDocumento())) {
                            tipoServico = TipoServicoEnum.CANCELAMENTO;
                        }

                        if (tipoServico != null) {
                            String xmlProcessado = documentoClobService.getXmlRetornoByIdDocFiscalAndTipoServico(documentoFiscal.getId(), tipoServico);

                            if (xmlProcessado == null) {
                                xmlProcessado = documentoClobService.getMaxXmlEventoByIdDocFiscalAndTipoServico(documentoFiscal.getId(), tipoServico);
                            }

                            if (xmlProcessado != null) {
                                kafkaProducerService.invokeCallback(documentoFiscal, xmlProcessado, tipoServico);
                                LOGGER.info("gerarCallbackFaltante (scheduled) >> invokeCallback do documento fiscal {}", documentoFiscal.getId());
                            }
                        }
                    }

                } catch (Exception e) {
                    LOGGER.error("gerarCallbackFaltante (scheduled) - ERRO no documento fiscal {} {} ", documentoFiscal.getId(), e.getMessage(), e);
                }
            });
        ...
        LOGGER.info("gerarCallbackFaltante (scheduled) - FIM");
    }
    ...
    private List<DocumentoFiscal> getDocumentosElegiveis() {
        ...
        if (cacheLoteRepository.setIfAbsent(LOCK_EM_SELECAO_DE_DOCUMENTOS, true, 1, TimeUnit.MINUTES)) {
            ...
            try {
                ...
                List<DocumentoFiscal> listaDocu = documentoFiscalService.listByDateIntervalAndSituacaoAndNotExistsInev(TIPO_DOCUMENTO_FISCAL_NFE, cal.getTime(), cal2.getTime(), SituacaoEnum.LIQUIDADO, notIn);
    ...
    private void removerMembrosLista(List<DocumentoFiscal> lista) {
        LOGGER.info("gerarCallbackFaltante (scheduled) - removerMembrosLista {}", lista.size());
